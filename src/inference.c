...
void infer_free(Infer *infer)
{
  MapIndex index;
  if (infer) {
    for (map_iterator(infer->def_types, &index); map_next(infer->def_types, &index);) {
      type_free(map_value(infer->def_types, &index));
    }
    map_free(infer->def_types);
    for (map_iterator(infer->expr_types, &index); map_next(infer->expr_types, &index);) {
...
{
  MapIndex index;
  if (map_entry(infer->def_types, (void *) def, &index)) {
...
{
  MapIndex index;
  if (map_entry(infer->expr_types, (void *) node, &index)) {
...
{
  MapIndex index;
  map_entry(infer->expr_types, (void *) node, &index);
