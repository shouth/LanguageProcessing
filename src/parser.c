...
{
  return check(parser, TOKEN_KIND_IDENTIFIER) && keyword == parser->_symbol;
}

static int check_identifier(Parser *parser)
{
  return check(parser, TOKEN_KIND_IDENTIFIER) && !symbol_is_keyword(parser->_symbol);
}

static int eat(Parser *parser, TokenKind kind)
{
...
static void parse_array_type(Parser *parser)
{
  ...
  expect(parser, TOKEN_KIND_LEFT_BRACKET);
  expect(parser, TOKEN_KIND_INTEGER);
  expect(parser, TOKEN_KIND_RIGHT_BRACKET);
...
static void parse_variable(Parser *parser)
{
  ...
  if (eat(parser, TOKEN_KIND_LEFT_BRACKET)) {
...
static void parse_parenthesized_expression(Parser *parser)
{
  ...
  expect(parser, TOKEN_KIND_LEFT_PARENTHESIS);
  parse_expression(parser);
  expect(parser, TOKEN_KIND_RIGHT_PARENTHESIS);
  node_finish(parser);
}
...
static void parse_factor(Parser *parser)
{
  ...
  } else if (check(parser, TOKEN_KIND_LEFT_PARENTHESIS)) {
  ...
  } else if (!(eat(parser, TOKEN_KIND_INTEGER) || eat_keyword(parser, SYMBOL_KEYWORD_TRUE) || eat_keyword(parser, SYMBOL_KEYWORD_FALSE) || eat(parser, TOKEN_KIND_STRING))) {
    /* make error */
  }
}
...
{
  return check(parser, TOKEN_KIND_STAR)
...
{
  return check(parser, TOKEN_KIND_PLUS)
    || check(parser, TOKEN_KIND_MINUS)
...
{
  return check(parser, TOKEN_KIND_EQUAL)
    || check(parser, TOKEN_KIND_NOT_EQUAL)
    || check(parser, TOKEN_KIND_LESS_THAN)
    || check(parser, TOKEN_KIND_LESS_THAN_EQUAL)
    || check(parser, TOKEN_KIND_GREATER_THAN)
    || check(parser, TOKEN_KIND_GREATER_THAN_EQUAL);
}
...
static void parse_assignment_statement(Parser *parser)
{
  ...
  parse_variable(parser);
  expect(parser, TOKEN_KIND_ASSIGN);
  parse_expression(parser);
  node_finish(parser);
}
...
static void parse_actual_parameter_list(Parser *parser)
{
  ...
  do {
    parse_expression(parser);
  } while (eat(parser, TOKEN_KIND_COMMA));
  node_finish(parser);
}
...
static void parse_output_value(Parser *parser)
{
  ...
  parse_expression(parser);
  if (eat(parser, TOKEN_KIND_COLON)) {
    expect(parser, TOKEN_KIND_INTEGER);
  }
  node_finish(parser);
}
...
static void parse_compound_statement(Parser *parser)
{
  ...
  do {
    parse_statement(parser);
  } while (eat(parser, TOKEN_KIND_SEMICOLON));
...
static void parse_variable_declaration(Parser *parser)
{
  ...
  expect(parser, TOKEN_KIND_COLON);
  parse_type(parser);
  node_finish(parser);
}
...
static void parse_procedure_declaration(Parser *parser)
{
  ...
  expect_identifier(parser);
  if (check(parser, TOKEN_KIND_LEFT_PARENTHESIS)) {
    parse_formal_parameter_list(parser);
  }
  expect(parser, TOKEN_KIND_SEMICOLON);
